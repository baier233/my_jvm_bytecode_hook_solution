...
namespace jni
{
	...
	public:
		...
		bool is_same_object(jobject other) const
		{
			return get_env()->IsSameObject(object_instance, other) == JNI_TRUE;
		}
		...
		bool is_null() const
		{
			if (object_instance)
			{
				return get_env()->IsSameObject(object_instance, nullptr) == JNI_TRUE;
			}
			return true;
		}
		...
		jclass get_class_base_on_object() const{
			return get_env()->GetObjectClass(object_instance);
		}


		void print_object_name() const {
			auto klass = get_class_base_on_object();
			auto ik = java_hotspot::instance_klass::get_instance_class(klass);
			auto name = ik->get_name()->to_string();
	...
	public:
		...
		array_element_type at(int index) {
			return get_env()->GetObjectArrayElement((jobjectArray)object_instance, index);
		}

		std::vector<array_element_type> to_vector_global_ref() const
		{
			jsize length = get_length();
			std::vector<array_element_type> vector{};
			if (length <= 0) vector;
			...
			{
				for (jsize i = 0; i < length; ++i)
					vector.push_back(array_element_type(get_env()->GetObjectArrayElement((jobjectArray)object_instance, i), true));
			}
			if constexpr (std::is_same_v<jboolean, array_element_type>)
			{
				std::unique_ptr<jboolean[]> buffer = std::make_unique<jboolean[]>(length);
	...
	public:
		...
		static void reinit() {
			jclass_cache<klass>::value = nullptr;
			jclass_cache<klass>::value = get_cached_jclass<klass>();
			klass tmp{};
		}
	};
