...
java_hotspot::java_thread* java_hotspot::java_thread::current()
{
	return reinterpret_cast<java_thread*>(thread_base::current());
}
...
auto java_hotspot::java_thread::get_method() -> java_hotspot::method*
{
	static VMStructEntry* _anchor_entry = JVMWrappers::find_type_fields("JavaThread").value().get()["_anchor"];
	return *reinterpret_cast<method**>(*(reinterpret_cast<int64_t**>(reinterpret_cast<uint8_t*>(this) + _anchor_entry->offset + 0x10)) - 3);
}

void java_hotspot::java_thread::set_method(method* m)
{
	static VMStructEntry* _anchor_entry = JVMWrappers::find_type_fields("JavaThread").value().get()["_anchor"];
	*reinterpret_cast<method**>(*(reinterpret_cast<int64_t**>(reinterpret_cast<uint8_t*>(this) + _anchor_entry->offset + 0x10)) - 3) = m;
}
