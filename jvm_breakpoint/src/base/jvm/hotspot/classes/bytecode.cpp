...
#include <hotspot/break/jvm_break_points.h>
...
namespace java_runtime {
	...
	bytecodes bytecode::get_opcode() const {
		if (static_cast<bytecodes>(*this->opcode) == bytecodes::_breakpoint)
		{
			return static_cast<bytecodes>(jvm_break_points::original_bytecodes[reinterpret_cast<uintptr_t>(this->opcode)]);
		}
