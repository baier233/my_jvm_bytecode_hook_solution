...
public:
	template <typename T>
	static T as(void* address) {
		return *static_cast<T*>(address);
	}

	template <typename T>
	static T as(void* address, const uintptr_t obj_offset) {
		return *reinterpret_cast<T*>(reinterpret_cast<uintptr_t>(address) + obj_offset);
	}
};
class VMWrapper {
public:
	VMWrapper(VMTypeEntry* type_entry, JVMWrappers::struct_entry_t* fields, void* obj)
		: _type_entry(type_entry), _fields(fields), _obj(obj) {}

	static VMWrapper* as(const char* type_name, void* obj);

	template <typename T>
	T* get_field_ptr(const char* field_name) {
		const auto field_address = find_field_address(field_name);
		if (field_address == nullptr) {
			throw std::invalid_argument("field address not found");
		}
		return static_cast<T*>(field_address);
	}

	template <typename T>
	T get_field(const char* field_name) {
		const auto field_address = find_field_address(field_name);
		if (field_address == nullptr) {
			throw std::invalid_argument("field address not found");
		}
		return OopObj::as<T>(field_address);
	}

	template <typename T>
	void set_field(const char* field_name, T value) {
		const auto field_address = find_field_address(field_name);
		if (field_address == nullptr) {
			throw std::invalid_argument("field address not found");
		}
		*static_cast<T*>(field_address) = value;
	}

	void* find_field_address(const char* field_name) {
		const auto& field_entry = (*_fields)[field_name];
		if (field_entry == nullptr) {
			return nullptr;
		}
		if (field_entry->isStatic) {
			return field_entry->address;
		}
		return reinterpret_cast<void*>(reinterpret_cast<uintptr_t>(this->_obj) + field_entry->offset);
	}

private:
	VMTypeEntry* _type_entry;
	JVMWrappers::struct_entry_t* _fields;
	void* _obj;
};
namespace global_offsets {
	inline uintptr_t klass_offset = 0;
	inline int* tls_index_address = nullptr;
}
