...
extern "C" {
	...
	uintptr_t __forceinline jhook_get_rax_address();
}
...
auto _declspec(noinline) jvm_break_points::breakpoint_callback(
	...
	breakpoint_handler(java_thread, method, bytecode_address, parameters, 0);
}
...
auto jvm_break_points::breakpoint_handler(
	...
	native_callback_t* callback = nullptr;
	{
		std::lock_guard<std::mutex> lock(callback_mutex);
		callback = &breakpoint_callbacks[bytecode_address];
	}
	if (!callback) {
	...
	const auto breakpoint_info = std::make_unique<break_point_info>(
		method, bytecode_address, java_thread, parameters, result_val_addr
	);
...
auto jvm_break_points::original_bytecode_handler(
	...
	{
		std::lock_guard<std::mutex> lock(jvm_internal::spoofed_original_bytecodes_mutex);
	...
	std::lock_guard<std::mutex> lock(o_bytecode_mutex);
